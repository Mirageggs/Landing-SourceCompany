...
export function ServicesSection() {
  return (
    <section id="servicios" className="py-32 bg-black text-white">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {services.map((service, index) => (
            <div
              key={index}
              className={`reveal group relative flex flex-col justify-between p-8 rounded-2xl bg-[#0a0a0a] border border-white/5 hover:border-[#78B803]/40 transition-all duration-500 overflow-hidden min-h-[220px] ${service.span}`}
              style={{ transitionDelay: `${0.1 * index}s` }}
            >
              <div className="flex items-start justify-between mb-8">
                <span className="text-3xl text-[#78B803]/40 group-hover:text-[#78B803] transition-colors duration-500">
                  {service.icon}
                </span>
